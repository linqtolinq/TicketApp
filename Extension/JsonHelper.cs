...
namespace MQTTDashWebApi.Extension
{
    public static class JsonHelper
    {
        ...
        /// <summary>
        /// 将JSONstring转换回对象class【对象数组】
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="JsonStr"></param>
        /// <returns></returns>
        public static T GetModelFromJsonStringFromList<T>(this string JsonStr, string timeFormatStr= TIME_FORMAT_FULL)
        {
            try
            {
                var serializerSettings = new JsonSerializerSettings
                ...
                T mouldNameList = JsonConvert.DeserializeObject<T>(JsonStr, serializerSettings);
                return mouldNameList;
            }
            catch (Exception ex)
            {
                Console.WriteLine(ex.Message + "=>" + ex.StackTrace);
